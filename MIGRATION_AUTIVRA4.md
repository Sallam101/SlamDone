# Autivra4 → SupeSlam migration

SupeSlam uses a **complete private migration JSON** generated from the final Autivra SQLite database. This is intentionally more complete than the legacy Autivra backup JSON because it can also preserve journal-version history and timer state.
...
Large embedded NorthStar images remain intact. SupeSlam removes the base64 image from the main note document during cloud upload, stores it as bounded `northstar_assets/{noteId}/chunks/*` documents under the same UID, and reassembles it into local SQLite on each device.
...
Device-specific legacy transport settings are deliberately not imported: the old device ID, Google Drive sync folder/mode, and floating-Windows-timer command/heartbeat belong to Autivra's native transport layer. A migrated floating timer is reassigned to SupeSlam's main web timer while its timing state is preserved.

## Import steps

1. Keep the private migration JSON somewhere outside the GitHub repository.
2. Open `https://Sallam101.github.io/SupeSlam/`.
3. Sign in with the Google account that should own the SupeSlam data.
...
7. SupeSlam validates the format, version, source counts, and source checksum before applying it.
