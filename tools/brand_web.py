...
(() => {
  ...
  const colors = ['#78D12F', '#2457D6', '#00897B', '#7B1FA2', '#C62828', '#EF6C00', '#455A64', '#AD1457'];
  let pipWindow = null;
  let snapshot = {};
  let ticker = null;
  let audio = null;
  ...
  const render = () => {
    ...
    const accent = colors[Math.max(0, Math.min(colors.length - 1, Number(snapshot.colorIndex) || 0))];
    ...
    doc.head.innerHTML = `<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        #sd-timer-root { --accent: #78D12F; width: 100%; height: 100%; min-width: 156px; min-height: 150px; display: grid; grid-template-rows: 32px 1fr; overflow: hidden; border-radius: 14px; background: color-mix(in srgb, Canvas 94%, var(--accent) 6%); color: CanvasText; border: 1px solid color-mix(in srgb, var(--accent) 30%, transparent); transition: opacity 120ms ease; }
        ...
        .dial-inner { width: 100%; height: 100%; border-radius: 50%; display: grid; place-content: center; text-align: center; background: Canvas; border: 1px solid color-mix(in srgb, var(--accent) 12%, transparent); }
        ...
        .popover { position: absolute; z-index: 5; top: 3px; left: 7px; right: 7px; min-height: 38px; padding: 5px 8px; border-radius: 10px; background: color-mix(in srgb, Canvas 94%, var(--accent) 6%); border: 1px solid color-mix(in srgb, var(--accent) 26%, transparent); box-shadow: 0 5px 18px rgba(0,0,0,.18); display: none; align-items: center; gap: 8px; }
